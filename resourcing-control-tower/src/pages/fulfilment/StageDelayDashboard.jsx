...
export default function StageDelayDashboard() {
  ...
  const [filters, setFilters] = useState({ statuses: [], customers: [], pms: [] });
  ...
  const stageSummaryRows = useMemo(() => buildStageSummary(filteredRows.filter((row) => !row.delayExcluded)), [filteredRows]);

  const options = useMemo(() => ({
    statuses: uniqueOptions(delayRows, 'demandStatus'),
    customers: uniqueOptions(delayRows, 'customer'),
    pms: uniqueOptions(delayRows, 'pm')
  }), [delayRows]);
  ...
  return (
    <main className="page">
      ...
      {loading ? (
        ...
        <>
          <section className="fulfilment-filter-bar stage-delay-filter-bar">
            ...
            <div className="filter-action-cell">
              <Button variant="outlined" onClick={() => setFilters({ statuses: [], customers: [], pms: [] })}>Clear Filters</Button>
            </div>
          </section>

          <section className="split-page stage-delay-panels">
            <div className="plain-section">
              <div className="page-title-row compact-row">
                <div>
                  <h2>Delay by Stage</h2>
                  <p>Candidate count and average days in the current stage.</p>
                </div>
              </div>
              <div className="fulfilment-block-heatmap snapshot-blocks">
                {stageSummaryRows.map((row) => (
                  <div className={`fulfilment-heat-block ${row.level}`} key={row.stageName}>
                    <strong>{row.delayed}</strong>
                    <span>{row.stageName}</span>
                    <small>{row.avgDelayDays}d avg | {row.spoc}</small>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="plain-section">
              <div className="page-title-row compact-row">
                <div>
                  ...
                  <p>Month-wise TAG profiles for the selected demand status, customer, and PM filters.</p>
                </div>
              </div>
              <div className="fulfilment-block-heatmap snapshot-blocks">
